...
int if_pjt_yhccl_content;
int pjt_fd;
long long pjt_memory_size;
...
void yhccl_contexts::init_large_msg_allreduce_buffer(int intra_node_rank, int intra_procn, int inter_node_rank)
{
    ...
    if (_opt.pp_node == -1)
        sprintf(name, "YCCL-daiy-%s", host_name);
    else
        sprintf(name, "YCCL-daiy-%s-%d", host_name, inter_node_rank);
    ...
    if_pjt_yhccl_content = true;
    pjt_memory_size = memory_sz;
    pjt_fd = fd;
    pjt_larger_msg_allreduce_shareM = larger_msg_allreduce_shareM;
