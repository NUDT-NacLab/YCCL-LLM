...
#ifdef YIZIMI_AUTO_PARA_OUTPUT
		if (ctx->global_rank == 0) {
			printf("[YCCL-LLM] procn=%d count=%d auto tuning over.\n", ctx->global_procn, size);
			printf("[YCCL-LLM] auto tuning A = %d, B = %d C = procn/A/B = %d.\n", ctx->_opt.pp_zni, ctx->_opt.pp_chip, ctx->global_procn / ctx->_opt.pp_zni / ctx->_opt.pp_chip / ctx->intra_node_procn);
		}
#endif
...
#ifdef YIZIMI_AUTO_PARA_OUTPUT
		if (ctx->global_rank == 0) {
			printf("[YCCL-LLM] procn=%d count=%d auto tuning off.\n", ctx->global_procn, size);
			printf("[YCCL-LLM] default A = %d, B = %d.\n", ctx->_opt.pp_zni, ctx->_opt.pp_chip);
		}
#endif
	}
}
